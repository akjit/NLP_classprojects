...
def run(Xtrain_file, Ytrain_file, test_data_file, pred_file):
    ...
    l2_decay = 0.00014                           #l2 regularization lambda constant
    #l2_decay = .0005
    #l2_decay = .001517
    ...
    weights = gradiant_descent(training,weights,learning_rate,lr_decay,l2_decay)

    predictions = np.zeros((test_data.shape[0],1))

    for tester in range(test_data.shape[0]):
        p = sigmoid(weights,test_data[tester])#,0:test_percent.shape[1]-1])
        if(p > (1-p)):
    ...
    # correct = 0.0
    # for guess in range(ytrain.shape[0]):
    #     if predictions[guess] == ytrain[guess]:
    #         correct = correct + 1.0

    # accuracy = correct/(ytrain.shape[0])
